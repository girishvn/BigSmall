...
# MODELS
from neural_methods.model.BigSmall_models import BigSmallSlowFastWTSM



class BigSmallMultitaskTrainer(BaseTrainer):

    def define_model(self, config):

        model = BigSmallSlowFastWTSM(out_size=len(config.DATA.TRAIN.LABELS.USED_LABELS), n_segment=3)

        # TODO - Add using TSM to config file or somewhere else
        self.using_TSM = True
        if self.using_TSM:
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                # au_labels = labels[:, self.label_idx_train_au, 0]
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            for _, test_batch in enumerate(data_loader['test']):
                ...
                # I think this may be because TSM data formatting is cropping data... not sure
                if labels.shape[0] == 0:
                    continue
                ...
                for idx in range(batch_size):
                    ...
                    #TODO USE THIS FOR DISFA DATASET
        ...
        if self.config.MODEL_SPECS.TEST.AU_METRICS: # run metrics, if not empty list
            ...
            # au_metric_dict = calculate_DISFA_au_metrics(preds_dict_au, labels_dict_au, self.config)
            if self.save_metrics:

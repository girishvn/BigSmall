...
class BaseTrainer:
    ...
    def load_model_weights(self, model_path, reset_dense_layers=True, freeze_other_weights = False):
        pass
    ...
    def __init__(self, config, data_loader):
        ...
        # DEFINING MODEL AND SENDING MODEL TO DEVICE
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            for _, test_batch in enumerate(data_loader['test']):
                ...
                #labels = labels[:, 0] # TODO use bpwave as label for BVP pseudo input
        ...
        if self.config.MODEL_SPECS.TEST.BVP_METRICS: # run metrics, if not empty list
            preds_dict, labels_dict = self.reform_preds_labels(preds_dict, labels_dict) # reform data
            print('BVP Metrics:')
        ...
        elif self.config.MODEL_SPECS.TEST.RESP_METRICS: # run metrics, if not empty list
            preds_dict, labels_dict = self.reform_preds_labels(preds_dict, labels_dict) # reform data
            print('Resp Metrics:')
        ...
        elif self.config.MODEL_SPECS.TEST.AU_METRICS: # run metrics, if not empty list
            preds_dict, labels_dict = self.reform_preds_labels(preds_dict, labels_dict, flatten=False) # reform data
            print('AU Metrics:')

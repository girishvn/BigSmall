...
# METRICS
from multitask_eval.metrics import (calculate_bvp_metrics, calculate_resp_metrics, 
                                    calculate_au_metrics, calculate_DISFA_au_metrics)


# MODELS 
from neural_methods.model.BigSmall_models import (BigSmallSlowFastWTSM, BigSmallWTSM, BigSmallSlowFastTSM, 
                                               BigSmallSlowFast, BigSmallBaseModel, 
                                               BSSFWTSM_ConcatFusion, BSSFWTSM_BiDirLatConn, 
                                               BSSFWTSM_Big2SmallLatConn, BSSFWTSM_Small2BigLatConn, 
                                               BigSmallSlowFastWTSM_GrayScaleBig)

class BigSmallMultitaskTrainer_Pretrain(BaseTrainer):

    def define_model(self, config):

        # Note: Standard Big and Small Pathways are set in BigSmallTrainer (single task trainer module)

        # FINAL BIGSMALL MODEL
        model = BigSmallSlowFastWTSM(out_size=len(config.DATA.TRAIN.LABELS.USED_LABELS), n_segment=3)

        # ABLATION MODELS
        # model = BigSmallSlowFastWTSM_SummedFeatMapsSinglePool(out_size=len(config.DATA.TRAIN.LABELS.USED_LABELS), n_segment=3)
        # model = BigSmallSlowFastTSM(out_size=len(config.DATA.TRAIN.LABELS.USED_LABELS), n_segment=3)
        # model = BigSmallSlowFast(out_size=len(config.DATA.TRAIN.LABELS.USED_LABELS), n_segment=3)
        # model = BigSmallWTSM(out_size=len(config.DATA.TRAIN.LABELS.USED_LABELS))
        # model = BigSmallBaseModel(out_size=len(config.DATA.TRAIN.LABELS.USED_LABELS))

        # ALTERNATIVE FUSION
        # model = BSSFWTSM_ConcatFusion(out_size=len(config.DATA.TRAIN.LABELS.USED_LABELS), n_segment=3)
        # model = BSSFWTSM_BiDirLatConn(out_size=len(config.DATA.TRAIN.LABELS.USED_LABELS), n_segment=3)
        # model = BSSFWTSM_Big2SmallLatConn(out_size=len(config.DATA.TRAIN.LABELS.USED_LABELS), n_segment=3)
        # model = BSSFWTSM_Small2BigLatConn(out_size=len(config.DATA.TRAIN.LABELS.USED_LABELS), n_segment=3)

        # EXPERIMENTAL
        # model = BigSmallSlowFastWTSM_GrayScaleBig(out_size=len(config.DATA.TRAIN.LABELS.USED_LABELS), n_segment=3)
    ...
    def format_data_shape(self, data, labels):
        ...
        # If using TSM modules - reshape for GPU
        if self.using_TSM:
    ...
    def __init__(self, config, data_loader):
        ...
        print('')
        print('Init Multitask Pretrain Trainer')
        print('')
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            # MODEL PRETRAINING TODO: Implement this into the config file....
            # if epoch == 0:
            ...
            for idx, batch in enumerate(tbar):
                ...
                # TODO Used to visualize task gradients... clean this up...
                # Used to visualize tasks gradients...
                # if epoch == 1:
                #     losses = [au_loss, bvp_loss, resp_loss]
                #     assert len(losses) == self.num_tasks
                #     _ = self.optimizer.pc_backward_return_grads(losses)
            ...
            # TODO uncomment for PRETRAINING... integrate this more cleanly...
            # pretrain_loss_dict[epoch] = pretrain_loss
            # pretrain_bvp_loss_dict[epoch] = pretrain_bvp_loss
            # pretrain_resp_loss_dict[epoch] = pretrain_resp_loss
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            for _, test_batch in enumerate(data_loader['test']):
                ...
                # Weird dataloader bug is causing the final training batch to be of size 0... TODO THIS IS A BANDAID
                if labels.shape[0] == 0:
                    continue
                ...
                for idx in range(batch_size):
                    ...
                    # labels_au = labels_au[subsamle_idx_arr, :]
                    # preds_dict_au[subj_index][sort_index] = au_out[idx * int(self.chunk_len/3):(idx + 1) * int(self.chunk_len/3)]
                    # labels_dict_au[subj_index][sort_index] = labels_au[idx * int(self.chunk_len/3):(idx + 1) * int(self.chunk_len/3)]
        ...
        if True:
            ...
            # Change chunk length to be test chunk length
            self.chunk_len = config.DATA.PRETRAIN.PREPROCESS.CHUNK_LENGTH 
            ...
            # MODEL TESTING
            # TODO TESTING ON SECOND DATASET - THIS SHOULD NOT BE NAMED THE PRETRAIN SET... NEED TO CLEAN THIS UP...
            with torch.no_grad():
        ...
        if self.save_data and self.save_train:
            print('SAVING PREDICTIONS ON TRAINING DATA IS CURRENTLY NOT SUPPORTED...')

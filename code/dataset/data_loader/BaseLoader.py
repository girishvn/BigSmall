...
class BaseLoader(Dataset):
    ...
    def __init__(self, name, data_path, config_data, total_config):
        ...
        self.data_augmentation = total_config.MODEL_SPECS.TRAIN.DATA_AUG
    ...
    @staticmethod
    def diff_max_min_normalize_label(label, max_val, min_val):
        norm_label = (label - min_val) / (max_val - min_val) # max min normalize
        diff_norm_label = np.diff(norm_label, axis=0) # diff label frames
        diff_norm_label[np.isnan(diff_norm_label)] = 0
        return diff_norm_label

        # diff_norm_std_label = diff_norm_label / np.std(diff_norm_label) # TODO  -  Do i need to divide by standard div
        #return diff_norm_std_label

    @staticmethod
    def max_min_normalize_label(label, max_val, min_val):
        norm_label = (label - min_val) / (max_val - min_val) # max min normalize
        norm_label[np.isnan(norm_label)] = 0
        return norm_label

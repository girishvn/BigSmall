...
def calc_fundementalfreq_peak(predictions, labels, diff_flag=True, signal='pulse', window_size=360, fs=30, bpFlag=True):
    ...
    if signal == 'pulse':
        ...
        raise ValueError('SIGNAL NOT SPECIFIED: PULSE OR RESP')
...
def calc_fundementalfreq_fft(predictions, labels, diff_flag=True, signal='pulse', fs=30, bpFlag=True):
    ...
    #N = pred_window.shape[1]
    f_prd, pxx_pred = scipy.signal.periodogram(
        pred_window, fs=fs, nfft=N, detrend=False)
    if signal == 'pulse': # regular Heart beat are 0.75*60 and 2.5*60
        fmask_pred = np.argwhere((f_prd >= 0.75) & (f_prd <= 2.5))
    elif signal == 'resp': # regular RR beat are 0.08*60 and 0.5*60
        # fmask_pred = np.argwhere((f_prd >= 0.08) & (f_prd <= 0.5))
        fmask_pred = np.argwhere((f_prd >= 0.13) & (f_prd <= 0.5))
    ...
    if signal == 'pulse':
        ...
        fmask_label = np.argwhere((f_label >= 0.13) & (f_label <= 0.5))
